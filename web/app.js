...
// Detect Capacitor / Mobile WebView
...
function isMobileWebView() {
    ...
    const androidWV = /\bwv\b/.test(ua) || (/\bAndroid\b/.test(ua) && ua.includes('Version/'));
    const iosWV = /(iPhone|iPad|iPod).+AppleWebKit(?!.*Safari)/i.test(ua);
    return androidWV || iosWV || isCapacitor();
}
...
async function signInWithGoogle() {
    try {
        ...
        const firebaseAuthPlugin =
            (typeof window !== 'undefined' && window.FirebaseAuthentication) ||
            (isCap && window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.FirebaseAuthentication) ||
            null;

        if (isCap && firebaseAuthPlugin) {
            try {
                const result = await firebaseAuthPlugin.signInWithGoogle();
                if (result && result.user) {
                    let idToken = (result.credential && result.credential.idToken) ? result.credential.idToken : null;
                    if (!idToken) {
                        const tokenRes = await firebaseAuthPlugin.getIdToken({ forceRefresh: true });
                        idToken = tokenRes && tokenRes.token ? tokenRes.token : null;
                    }
                    if (idToken) {
                ...
                console.warn('Native FirebaseAuthentication Google sign-in failed, falling back.', e);
            }
        }
...
async function addTransaction(e) {
    ...
    saveTransactionToFirebase(transaction).catch(console.error);
...
async function clearAllTransactions() {
    ...
    if (confirm('Are you sure you want to delete ALL transactions? This cannot be undone.')) {
        ...
        // Delete all from Firebase in background
        Promise.all(transactionsToDelete.map(t => deleteTransactionFromFirebase(t.id)))
            .catch(console.error);
    }
}
